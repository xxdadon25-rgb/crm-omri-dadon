...
export default function InvoicePDFPreview() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const invoiceData = await base44.entities.Invoice.get(invoiceId);
        if (!invoiceData) {
  ...
  const handlePrint = async () => {
    ...
    const pdfType = invoice.invoice_type === "monthly" ? "monthly_invoice" : "invoice";
    try {
      const blob = await generateDocumentPDF({ type: pdfType, doc: invoice, businessSettings });
  ...
  if (!invoice || !businessSettings) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-600 mb-2">Invoice Not Found</h1>
        </div>
      </div>
    );
  }
  ...
  const pdfType = invoice.invoice_type === "monthly" ? "monthly_invoice" : "invoice";
  const documentHTML = buildDocumentHTML({ type: pdfType, doc: invoice, businessSettings });
    ...
    <div className="min-h-screen bg-gray-100" style={{ direction: "rtl" }}>
      ...
      <div className="p-4 pb-12">
        <div className="max-w-6xl mx-auto bg-white shadow-lg">
          <div dangerouslySetInnerHTML={{ __html: documentHTML }} style={{ direction: "rtl" }} />
        </div>
      </div>
    </div>
  );
