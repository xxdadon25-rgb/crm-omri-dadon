              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item, i) => {
                    ...
                    const rowShortage = shortages.find(s => {
                      const sn = norm(s.name);
                      if (itemSku && sn === itemSku) return true;
                      if (sn === itemName) return true;
                      if (itemName.length >= 3 && (sn.includes(itemName) || itemName.includes(sn))) return true;
                      return false;
                    });
                    return (
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {rowShortage?.type === "unordered" && (
                            <span className="inline-flex items-center gap-1 text-xs text-purple-700 bg-purple-50 border border-purple-300 rounded px-2 py-0.5">
                              לא הוזמן
                            </span>
                          )}
                        </div>
